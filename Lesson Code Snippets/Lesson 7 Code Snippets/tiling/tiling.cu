...
const int NUMBLOCKS = 1;					// set this to 1 or 2 for debugging
...
__global__ void foo(float out[], float A[], float B[], float C[], float D[], float E[]){
	...
	__shared__ float s_in[BLOCKSIZE][5];
    int tid = threadIdx.x;
	...
	out[i] = (s_in[tid][0] + s_in[tid][1] + s_in[tid][2] + s_in[tid][3] + s_in[tid][4]) / 5.0f;
}

__global__ void bar(float out[], float in[])
{
    __shared__ float s_in[BLOCKSIZE];
    int tid = threadIdx.x;
	int i = tid + blockIdx.x*blockDim.x;
    s_in[tid] = in[i];
    __syncthreads();
	...
	out[i] = (s_in[tid-2] + s_in[tid-1] + s_in[tid] + s_in[tid+1] + s_in[tid+2]) / 5.0f;
}
...
int main(int argc, char **argv)
{
	...
	fooTimer.Start();
	foo<<<N/BLOCKSIZE, BLOCKSIZE, BLOCKSIZE*sizeof(float)>>>(d_fooOut, d_fooA, d_fooB, d_fooC, d_fooD, d_fooE);
	fooTimer.Stop();

	barTimer.Start();
	bar<<<N/BLOCKSIZE, BLOCKSIZE, BLOCKSIZE*sizeof(float)>>>(d_barOut, d_barIn);
	barTimer.Stop();
